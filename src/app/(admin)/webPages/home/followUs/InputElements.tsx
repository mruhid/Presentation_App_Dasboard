...
import { FollowUsMediaProps, FollowUsProps, SocialMediaProps } from "@/lib/type";
import { useQuery } from "@tanstack/react-query";
import { Loader2, Plus, Trash2Icon } from "lucide-react";
import { Dispatch, SetStateAction, useState } from "react";
import { useUpdateFollowUsMedia, useUpdateFollowUsTitle } from "./mutation";
import { redirect } from "next/navigation";

export default function InputElements({ data, type }: { data: FollowUsMediaProps[] | FollowUsProps, type: boolean }) {
    const [newData, setNewData] = useState<FollowUsMediaProps[] | FollowUsProps>(data);
    const mutation = useUpdateFollowUsTitle()
    const mutation1 = useUpdateFollowUsMedia()
    const isDisable = () => {
        ...
        <>
            <div className="sticky top-20 z-10 bg-secondary shadow-sm rounded-xl">
                <div className="flex justify-between items-center py-4">
                    <h1 className="p-4 mx-4 text-muted-foreground bg-card rounded-xl text-xl">Update followUs section</h1>
                    <div className="flex items-center justify-center gap-3">
                        {type && <div className="mx-4">
                            <AddSocialMedia elements={newData} setElements={setNewData} IDS={ids} />
                        </div>}
                        <div className="mx-4">
            ...
            <div className="flex flex-col gap-4 py-4">
                {!newData.length && <h1 className="text-destructive">FollowUs section no have media elements.You can use add buttton for this</h1>}
...
interface AddSocialMediaProps {
    IDS: number[];
    setElements: Dispatch<SetStateAction<FollowUsMediaProps[]>>;
    elements: FollowUsMediaProps[];
}
...
export function AddSocialMedia({ IDS, setElements, elements }: AddSocialMediaProps) {
    ...
    const handleMedia = (media: SocialMediaProps) => {
        const newElement: FollowUsMediaProps = {
        ...
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>You can add new social media platforms</DialogDescription>
                </DialogHeader>
                <DialogFooter>
