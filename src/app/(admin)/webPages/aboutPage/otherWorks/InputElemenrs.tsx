...
import { useUpdateOtherWorksMutation } from "./mutation";
import { redirect } from "next/navigation";

export default function InputElements({ data }: { data: OtherWorksSectionProps[] }) {
    const [newData, setNewData] = useState<OtherWorksSectionProps[]>(data);
    const mutation = useUpdateOtherWorksMutation()
    const isDisable = () => {
        return newData.some((item) =>
            Object.entries(item).some(([key, value]) => {
                if (key !== "id" && key !== "overlay") {
                    return typeof value === "string" && value.trim() === "";
                }

                if (key === "overlay" && typeof value === "object" && value !== null) {
        ...
        <>
            <div className="sticky top-20 z-10 bg-secondary shadow-sm rounded-xl">
                <div className="flex justify-between items-center py-4">
                    <h1 className="p-4 mx-4 text-muted-foreground bg-card rounded-xl text-xl">Update other works section</h1>
            ...
            <div className="flex flex-col gap-4 py-4">
                ...
                <div className="border-[3px] mb-5 shadow-sm border-secondary rounded-xl">
                    <Button className="my-4" variant={'outline'}>
                        <h1 className="text-primary text-lg font-semibold">Other works section&apos;s catalog</h1>
                    </Button>
                    {
                        Object.entries(newData[0].overlay).map(([key, value]) => {

                            return (
                                key !== "buttonSrc" && <div key={`${key}`} className="grid grid-cols-4 items-center gap-4 bg-card p-4 rounded">
                                    <Label htmlFor={`${key}`} className="text-left font-semibold capitalize">{key}</Label>
                                    {<Input type="text" id={`${key}`} name={key} value={String(value)} className="col-span-3 bg-secondary" onChange={(e) => handleOverlayInputChange(e, newData[0].id, key)} autoComplete="off" />
                                    }
                                </div>
                            );
                        })
