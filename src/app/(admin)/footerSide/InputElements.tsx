...
import { FooterSocialMediaProps, FooterTextProps, SocialMediaProps } from "@/lib/type";
import { useQuery } from "@tanstack/react-query";
import { Loader2, Plus, Trash2Icon } from "lucide-react";
import { Dispatch, SetStateAction, useState } from "react";
import { useUpdateFooterPlatformMutation, useUpdateFooterTextMutation } from "./mutation";
import { redirect } from "next/navigation";

export default function InputElements({ data, type }: { data: FooterSocialMediaProps[] | FooterTextProps[], type: boolean }) {
    const [newData, setNewData] = useState<FooterSocialMediaProps[] | FooterTextProps[]>(data);
    const mutation = useUpdateFooterTextMutation()
    const mutation1 = useUpdateFooterPlatformMutation()
    const isDisable = () => {
        ...
        setNewData((prevData) => {
            if (Array.isArray(prevData)) {
                return prevData.map((item) =>
                    item.id === id ? { ...item, [key]: key === "copyright" ? `&#169;${value}` : value } : item
                );
            }
        });
    };
        ...
        <>
            <div className="sticky top-20 z-10 bg-secondary shadow-sm rounded-xl">
                <div className="flex justify-between items-center py-4">
                    <h1 className="p-4 mx-4 text-muted-foreground bg-card rounded-xl text-xl">Update footer side</h1>
                            ...
                            {Object.entries(item).map(([key, value]) => {
                                ...
                                return (
                                    <div key={`${item.id}-${key}`} className="grid grid-cols-4 items-center gap-4 bg-card p-4 rounded">
                                        ...
                                        {!type ? <Input type="text" id={`${item.id}-${key}`} name={key} value={key === "copyright" ? String(value.split("169;")[1]) : String(value)} className="col-span-3 bg-secondary" onChange={(e) => handleInputChange(e, item.id, key)} autoComplete="off" /> :
...
interface AddSocialMediaProps {
    IDS: number[];
    setElements: Dispatch<SetStateAction<FooterSocialMediaProps[]>>;
    elements: FooterSocialMediaProps[];
}
...
export function AddSocialMedia({ IDS, setElements, elements }: AddSocialMediaProps) {
    ...
    const handleMedia = (media: SocialMediaProps) => {
        const newElement: FooterSocialMediaProps = {
